...
cbind(patients, my_matrix) #'combine columns.  R was forced to 'coerce' the numbers to characters, hence the double quotes. so numbers are just charactors now :(
my_data <- data.frame(patients, my_matrix) #include the names of our patients in the table without destroying the integrity of our numeric data
...
FUNCTIONS
Inputs to functions are often called arguments. The last R expression to be evaluated in a function will become the return value of that function.
...
########## anonymous functions  #########################
#pass a function as an argument without first defining the passed function 
...
The ellipses can be used to pass on arguments to other functions that are used within the function you're writing. Usually a function that has the
...
t4 <- strptime(t3, "%B %d, %Y %H:%M") #help R convert our date/time object to a format that it understands
Sys.time() -t1  # how much time has passe since t1 was created
difftime(Sys.time(), t1, units = 'days') # How many days since t1
