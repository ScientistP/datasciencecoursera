...
my_data <- data.frame(patients, my_matrix2) # method 4, include the names of our patients in the table without destroying the integrity of our numeric data
...
increment <- function(number, by = 1) (number + by) #Same as above
...
The idea behind a loop function is you want to execute a loop over an object or a set of objects in a way that's kind of that does a lot of work in, in a very small amount of space. That way, you don't have to type as much on the command line. 

lapply takes in 1) list 2) Function name 3) arguments

lapply and sapply 
apply() and sapply(). Both take a list as input, apply a function to each element of the list, then combine and return the result. lapply() always returns a list, whereas sapply() attempts to simplify the result. So if the result is a list with every element of length 1, then a vector is returned. If the result is a list where every elemet is a vector of length 5, then a matrix of 5xn is returned. 

See Hadley Wickham's Journal of Statistical Software paper titled 'The Split-Apply-Combine Strategy for Data Analysis'. lapply: list apply
...
df <- data.frame(patients = c("Jon", "Bill", "Maria", "Ben", "Tina"),
                 age = 1:5)
mean(df) #NA PROBLEM
lapply(df,mean)  # NA, 3 result is a list


x<- 1:4
lapply(x, runif) # runif generates a uniform random variables using a random number generator. So runif 2, it's going to generate a vector of two random variables

lapply(x, runif, min=0, max=10) #random variables limits

x <- list(a=1:4, b=rnorm(10), c=rnorm(20,1), d=rnorm(100,5))
lapply(x,mean)
sapply(x,mean) #knit presentation

```
apply: 
evaluates a function (often anonymous) in one line 
str(apply)
function(x,MARGIN, FUN,...)

x: an array
MARGIN: integer vector 
FUN: a function to be applied
... : other arguments

```{r}
x <- matrix(rnorm(200),20,10) #20x10 dimension
apply(x,2, mean) #take the mean of each coloumn: keep 2nd dimension (coloumns) and collapse the 1st dim
apply(x,1, sum) #calculate the sum of each row

#specialized fast apply functions!
rowSums= apply(x,1,mean)
rowMeans =apply(x,1,mean)
colSums
colMeans

apply(x,1, quantile, probs=c(0.25,0.75)) # determine the quantiles you want to calculate

a<- array(rnorm(2*2*10), c(2,2,10)) #list of 10 matrix of 2 by 2 dimension ( c= 2,2,10), that takes the random numbers that keep on repeating (rnorm(2*2*10))
apply(a,c(1,2), mean) # take mean of the 2*2 matrix over the 3rd dimension 
rowMeans(a, dim=2) # same as above


```
mapply
multivariaate apply of sorts which applies in parallel over a set of arguments
```{r}
list(rep(1,4), rep(2,3), rep(3,2),rep(4,))
...
noise(5,1,2) #5 random variable, mean=2, sd=2


noise(1:5,1:5,2) # what I want to happen is to have one random normal with mean 1, two random normals with mean 2, three random normals with mean 3, etc, and then five random normals with mean 5. BUT doesn't work properly.Gives a vector of 5
mapply(noise, 1:5,1:5,2) # Now this works
list(noise(1,1,2), noise(2,2,2), noise(3,3,2), noise(4,4,2),noise(5,5,2)) #same as above

```
*tapply: apply a function over a subset of vectors
there are pieces of this vector that you want to calculate a summary statistic over.the first argument is a  vector of some sort. The second argument is another vector of the same length which identifies which group each element of the numeric vector is in.Forexampel: aybe the first 50 observations are men and the second 50 observations are women.if you want to take the mean of the numeric factor within men or within women, then you can use tapply to do this.
...
```{r}
x<- c(rnorm(10),runif(10),rnorm(10,1)) # a vector of normal random variables and uniform random variables and there's ten normals, ten uniforms, then ten normals that have a mean of one. Uniform distributions provide information about the uniform distribution on the interval from min to max. runif 2 generates a vector of two random variables

#So here we have three groups

f <- gl(3,10) #create another factor variable using the gl function with three levels. so three levels and each level is repeated 10 times.

tapply(x,f,mean) #tapply on x. pass it the factor variable f in the mean function, that allows me to take the mean of each group of numbers in, in x.
tapply(x,f,mean, simplify=FALSE) # gets a list

tapply(x,f, range)
```

split
splits a vector or objects into groups determined by a factor or a list of factors

str(split)
function(x,f,drop=FALSE,...)
x is a vector or a list
f is factor or a list of factors
drop indicates wether empty factors levels should be dropped

it is common to use split with lapply
```{r}
x<- c(rnorm(10),runif(10),rnorm(10,1)) # a vector of normal random variables and uniform random variables and there's ten normals, ten uniforms, then ten normals that have a mean of one. Uniform distributions provide information about the uniform distribution on the interval from min to max. runif 2 generates a vector of two random variables
...
x<- rnorm(10)
f1 <- gl(2,5) #e.g. gender levels: 2 levels, each level repeats 5 times
f2 <- gl(5,2) #e.g. race levels
interaction(f1,f2) #e.g. combination of gender and race
str(split(x,list(f1,f2))) #here we can see that some interactions create empty levels
str(split(x,list(f1,f2), drop=TRUE)) #the empty levels are dropped
```

SWIRL LESSONS
 http://archive.ics.uci.edu/ml/datasets/Flags
 flags.csv is saved in ~/dev/datasciencecoursera/data
 
Each of the *apply functions will SPLIT up some data into smaller pieces, APPLY a function to each piece, then
COMBINE the results. 

```{r}
flags <- read.csv("~/dev/datasciencecoursera/data/flags.csv")
class(flags) #data.frame. But What we really need is to call the class() function on each individual column. We can do it manually through a loop or using sapply and lappy functions.

cls_list <- lapply(flags, class) #step 1: every element returned is a character vector of length one (i.e. "integer" and "vector")
class(cls_list) #list, every element of the list is a character vector of length one
...
n example where sapply() can't figure out how to simplify the result and thus returns a list, no different from lapply()
...
sapply(unique_vals, length)  # the elements of the unique_vals list are all vectors of *different* length 
sapply(flags, unique) #this poses a problem for sapply() and it will be the same as lapply. 
lapply(unique_vals, function(elem) elem[2]) #you are interested in only the second item from each element of the unique_vals. elem is an 'anonymous functions'. Function takes one argument, elem, which is just a 'dummy variable' that takes on the value of each element of unique_vals, in turn.see below.
...
'anonymous functions'
lapply and the associated functions make heavy use of what, of what are called anonymous functions. Anonymous functions are functions that don't have names, so you don't assign them a name of some sort but you can kind of generate them on the fly. 
...
lapply(x, function(elt) elt[,1]) # used a made up function and names it elt which selects the first coloumn of each list. the function is not stored
...
vapply() 
1) as a safer alternative to sapply(), which is most helpful when writing your own functions.This is because sapply() tries to 'guess' the correct format of the result, vapply() allows you to specify it explicitly. If
the result doesn't match the format you specify, vapply() will throw an error, causing the operation to stop.  vapply() may perform faster than sapply() for large datasets. However, when doing data analysis interactively (at the prompt), sapply() saves you some typing and will often be good enough.
...
```{r}
vapply(flags, unique, numeric(1)) #says that you expect each element of the result to be a numeric vector of length 1. Since this is NOT actually the case, YOU WILL GET AN ERROR.
sapply(flags, unique) # return a list containing one vector of unique values for each column of the flags dataset
vapply(flags, class, numeric(1)) #says that you expect each element of the result to be a numeric vector of length 1. Since this is NOT actually the case, YOU get error.
sapply(flags, class)
...
#TAPPLY SPLITS
tapply(flags$animate, flags$landmass, mean) # mean of animate for each landmass. to apply the mean function to the 'animate' variable separately for each of the six landmass groups, thus giving us the proportion of flags containing an animate image WITHIN each landmass group. (landmass = 1) corresponds to North America and contains the highest proportion of flags with an animate image (0.4194).


tapply(flags$population, flags$red, summary) #4 million is the median population (in millions) for countries *without* the color red on their flag
